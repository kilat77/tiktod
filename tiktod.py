...
clear()
system('KILAT77 MARKETING TIKTOK BOT')

print(pyfiglet.figlet_format("KILAT77 TIKTOK BOT", font="slant"))
print("1. Viewbot.\n2. Heartbot.\n3. Followerbot.\n3. Sharebot.\n4. Credits.\n")

auto = int(input("Mode: "))

if auto == 1 or auto == 2 or auto == 3 or auto == 4:
    vidUrl = input("URL nya boskuu: ")
...
def title1(): # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title KILAT77 ^| Views Sent: {beautify(Views)} ^| Elapsed Time: {time_elapsed}')
...
def title2(): # Update the title IF option 2 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title KILAT77 ^| Hearts Sent: {beautify(Hearts)} ^| Elapsed Time: {time_elapsed}')
...
def title3(): # Update the title IF option 3 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title KILAT77 ^| Followers Sent: {beautify(Followers)} ^| Elapsed Time: {time_elapsed}')
...
def title4(): # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title KILAT77 ^| Shares Sent: {beautify(Shares)} ^| Elapsed Time: {time_elapsed}')
...
print(pyfiglet.figlet_format("KILAT77", font="slant"))
print("Log:")
...
if auto == 1:
    ...
    print("KILAT77 MARKETING TIKTOK BOT")
